...
srsinPal <- brewer.pal(name="Reds",n=9)[2:9] #omit the first color b/c v light
...
```
You can also download reference occurrences for all target taxa in one file <a href="https://NorthAmericanFruitNutTreeCWR.github.io/docs/references" target="_blank">here</a>.
...
if(print.message){print("There are no ex situ conservation collections reporting this taxon.")}
...
Contact data@bgci.org to request ex situ accessions data with all fields, including source and wild locality information.

<br>

### Map of ex situ geographical representativeness (GRS ex situ)

Taxon's potential distribution, occurrence points surrounded by 50km buffers (estimated distribution), and germplasm collection points surrounded by 50km buffers (potential distribution conserved ex situ). Use the layers icon on the left side of the map to change the basemap and toggle layers on/off. Click each point for more information.
...
Taxon's potential distribution, with ecoregions highlighted where no ex situ conservation germplasm has been collected. Toggle on the occurrence point layer using the layers icon on the left side of the map.
...
if(length(gEco)!=0){ # & !is.na(gEco[1])
  ...
  print.message3 <- FALSE
} else {
  print.message3 <- TRUE
  map1 <- "All ecoregions are conserved ex situ."
}

suppressWarnings(map1)

#`r if(print.message3){"All ecoregions are conserved ex situ."}`
...
Taxon's potential distribution, with distribution occurring within existing protected areas highlighted. Toggle on the occurrence point layer using the layers icon on the left side of the map.
...
Taxon's potential distribution, with ecoregions highlighted where no protected areas are present. Toggle on the occurrence point layer using the layers icon on the left side of the map.
...
if(length(gEco)!=0){
  if(length(gEco)>30){ # if more than 30 ecoregions, tmap could not visualize as categories ('cat' style)
    map4=
      tm_shape(r2, name = "Taxon's potential distribution")+
        tm_raster(style = "pretty", palette = ersBPal, 
  ...
  print.message2 <- TRUE
  map4 <- "All ecoregions are conserved in situ."
}
...
```
A summary figure of all conservation scores for our 95 target taxa can be viewed <a href="https://NorthAmericanFruitNutTreeCWR.github.io/docs/references/summaryfig" target="_blank">here</a> and used to prioritize among taxa.
...
*Additional resources – including data sources, citations, and methods information – can be found <a href="https://NorthAmericanFruitNutTreeCWR.github.io/docs/references" target="_blank">here</a>.*
...
*This work was funded by the United States Botanic Garden. Emily Beckman Bruns was also supported by NSF ABI grant #1759759*
